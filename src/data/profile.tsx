      ...
      projects: [
        {
          ...
          summary: "Social network backend, team of 10. I built the backend and owned deployment.",
          highlights: [
            "Set up the NestJS backend from an empty repo and built the CI/CD on GitHub Actions: dev and staging on Docker Compose behind Nginx (dev with auto-renewed TLS), production on AWS ECS.",
            "Production deploys on demand, per service or all at once, never two runs in parallel, and waits until ECS reports the service stable. A dev deploy that fails its health check rolls back to the previous image by itself.",
            "Tuned the PostgreSQL connection pool and indexes, made email unique only among active accounts so deleted users can sign up again, and built a shuffled feed that paginates stably without storing any order.",
          ],
          technologies: ["NestJS", "TypeScript", "PostgreSQL", "Redis", "Docker", "GitHub Actions", "AWS ECS / ECR", "Nginx"],
        },
        {
          name: "Subscription billing",
          period: "08/2026 —",
          summary: "Rails billing for a publisher on Stripe and PayPal side by side, plus passkey sign-in on AWS Cognito.",
          highlights: [
            "Made payment handling idempotent: each plan change is claimed by one conditional UPDATE, so Stripe and PayPal webhook retries never apply it twice.",
            "Moved receipt PDFs from wkhtmltopdf to headless Chrome, and wrote the row and the file upload in one transaction, so a failed upload retries cleanly with no orphaned data.",
            "Wrote the proration logic as plain Ruby with no database or gateway, unit-tested in RSpec; checked against the client's 28 official cases, it caught 14 errors in the spec itself.",
          ],
          technologies: ["Ruby on Rails 8", "PostgreSQL", "Stripe", "PayPal", "Sidekiq", "RSpec", "AWS Cognito", "Heroku"],
        },
        {
          name: "Factory planning system",
          period: "08/2025 —",
          summary: "Production scheduling, line simulation and live machine data, split across several Django services.",
          highlights: [
            "Put schedule generation behind a Redis lock so two triggers can never build the same schedule at once, and moved heavy aggregation into Celery background jobs.",
            "Extended the Django models package that three services share as a versioned wheel, and carried its schema migrations through all of them.",
            "Made bulk updates atomic, integrated an external system over GraphQL, and worked on the Java OPC-UA client that reads machine data.",
          ],
          technologies: ["Python", "Django", "Celery", "Redis", "PostgreSQL", "Docker", "Java", "OPC-UA", "GraphQL"],
        },
        {
          name: "Exam-practice app",
          period: "07/2025 — 10/2025",
          highlights: [
            "Migrated the backend from PostgreSQL to Firestore, with batch import and export scripts for the question bank.",
            "Cut per-request work: stats update incrementally instead of recomputing a whole category, and Redis caches stats and exam history.",
          ],
          technologies: ["Python", "FastAPI", "Firestore", "Firebase Auth", "Redis"],
        },
      ],
    },
    {
